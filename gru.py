...
class GRUCell(nn.Module):
    ...
    def _create_input_hidden_weights(self):
        return nn.Linear(self.input_size, 3 * self.hidden_size, self.bias).to(
            self.device
        )

    def _create_hidden_hidden_weights(self):
        return nn.Linear(self.hidden_size, 3 * self.hidden_size, self.bias).to(
            self.device
        )
    ...
    def forward(self, input, hx):
        ...
        reset_gate = torch.sigmoid(input_part[:, :hsize] + hidden_part[:, :hsize])
        update_gate = torch.sigmoid(
            input_part[:, hsize : 2 * hsize] + hidden_part[:, hsize : 2 * hsize]
        )
        new_gate = torch.tanh(
            input_part[:, 2 * hsize :] + reset_gate * hidden_part[:, 2 * hsize :]
        )
...
class GRU(nn.Module):
    def __init__(
        self, input_size, hidden_size, num_layers, device, bias=True, log_grads=False
    ):
...
class TTGRU(GRU):
    def __init__(
        ...
        self.new_core = new_core
        super().__init__(
            input_size, hidden_size, num_layers, device, bias, log_grads=log_grads
        )
