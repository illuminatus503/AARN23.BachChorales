...
def sample_TonicNet_random(load_path, max_tokens=2999, temperature=1.0):
    ...
    for i in range(max_tokens):
        ...
        y_hat = (
            model(x, z=psx_t, sampling=True, reset_hidden=reset_hidden)
            .data.view(-1)
            .div(temperature)
            .exp()
        )
...
def sample_TonicNet_beam_search(load_path, max_tokens=2999, beam_width=10, alpha=1.0):
    ...
    for i in range(max_tokens):
        ...
        for b in range(beam_width):
            ...
            if candidate_seqs[b][-1].item() == 0:
                ...
                log_probs[(98 * b) : (98 * (b + 1))] = (
                    logsoftmax(y_hat[0, 0, :]) + scores[b]
                )
        ...
        for b2 in range(beam_width):
            if not updated[b2]:
                ...
                if candidate_seqs[b2][-1].item() != 0:
                    ...
                    candidate_seqs[b2] = deepcopy(
                        candidate_seqs[rejection_reconciliation[b2]]
                    )
    ...
    normalised_scores = [
        scores[n] * (1 / (len(candidate_seqs[n]) ** alpha)) for n in range(beam_width)
    ]
